<?php
  ...
  function getList(){
    include "conect.php";
    $query = "SELECT user.* ,department.name AS dn, user.id AS u_id FROM user INNER JOIN department ON (user.id_dept = department.id) ORDER BY user.name";
    $consult = mysql_query($query, $con);
    if (!$consult) echo "<script>alert('falha na query');</script>";
    $button = "<td><div class='removeButton' onclick='removeUser(";

    while($line = mysql_fetch_array($consult))
      echo "<tr style='cursor:pointer;'><td>".$line["name"]."</td><td>".$line["type"]."</td><td>".$line["dn"]."</td>$button".$line["u_id"].")'>Remover</div></td></tr>";
