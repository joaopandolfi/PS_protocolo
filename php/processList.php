<?php
	...
	function getList(){
		include "conect.php";
		$query = "SELECT process.*, user.name AS un, department.name AS dn, process.id AS p_id FROM process INNER JOIN user ON (process.owner = user.id) INNER JOIN department ON (process.department = department.id)";
		$consult = mysql_query($query, $con);
		if (!$consult) echo "<script>alert('falha na query');</script>";
		$button = "<td><div class='removeButton' onclick='removeProcess(";

		while($line = mysql_fetch_array($consult))
		  echo "<tr style='cursor:pointer;' onclick='seeDetail(".$line["p_id"].")'><td>".$line["protocol"]."</td><td>".$line["un"]."</td><td>".$line["dn"]."</td>$button".$line["p_id"].")'>Remover</div></td></tr>";
